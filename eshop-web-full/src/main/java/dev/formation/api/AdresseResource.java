...
public class AdresseResource {
	...
	public Response get(@PathParam("id") Long id) {
		...
		AdresseDto adresseDTO = modelMapper.map(optAdresse.get(), AdresseDto.class);

		Optional<Long> optClient = personneRepository.findClientIdByAdresseId(id);

		if (optClient.isEmpty()) {
			Optional<Long> optFounisseur = personneRepository.findFournisseurIdByAdresseId(id);
			adresseDTO.setIdFournisseur(optFounisseur.get());
		} else {
			adresseDTO.setIdClient(optClient.get());
		}
	...
	public Response post(Adresse adresse, @Context UriInfo uriInfo) {
		try {
			...
			if (adresse.getId() != null) {
				URI location = uriInfo.getRequestUriBuilder().path("" + adresse.getId()).build();
